...
`+'\n```\n$not_math$\n```';
 ...
 page.on('dialog',d=>d.accept());await page.locator('#imageGallery button').click();await page.waitForFunction(()=>document.querySelector('#imagesStatus').textContent.includes('Nema'));await page.locator('.message-image').waitFor({state:'detached'});await page.locator('#settingsClose').click();
 ...
 await page.locator('#documentLibrary button').click();await page.waitForFunction(()=>document.querySelector('#filesStatus').textContent.includes('Nema'));
